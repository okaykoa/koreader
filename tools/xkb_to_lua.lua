...
local function render_layout(entries, layout, variant, layout_name)
    local lines = {
    ...
    for _, key in ipairs(keys) do
        ...
        for _, level in ipairs(LEVEL_MASKS) do
            if levels[level] then
                table.insert(rendered_levels, ("[%d] = %s"):format(level, lua_string(levels[level])))
            end
        end
...
local output = render_layout(entries, arguments.layout, arguments.variant, arguments.layout_name or arguments.layout)
